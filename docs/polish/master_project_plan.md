...
### Post–Phase 21 backlog (roadmap)

Work intentionally **outside** the polish sprint freeze. Detail lives in [final_verification_checklist.md](./final_verification_checklist.md) (section **Post–Phase 21 backlog**).

- **Safe Transfer Execution** — validate recommendation and operator/service readiness; invoke Twilio bridge via controlled server path; persist **`call_session`** transfer status; audit attempts; dedupe; never LLM-direct bridge.
- **Real operator roster / persistence** beyond advisory env toggles.
- **Full `agent_runs` / `tool_results` persistence** and **agent replay**.
- **Triage evaluation harness** (offline regression).
- **Real responder DB + ETA routing** (replace mocks).
- **Auth / RBAC / security hardening** (including webhook signature enforcement in production).
